...
class CLIUSBCameraTesterInstaller:
    ...
    def download_application(self):
        ...
        try:
            ...
            self.log_message(f"Download error encountered: {e}")
            self.log_message("Attempting download with unverified SSL context...")
            ...
            try:
                ...
                self.log_message(f"SSL fallback failed: {e2}")
                self.log_message("Attempting download with curl as final fallback...")
                ...
                if result.returncode != 0:
                    raise Exception(f"All download methods failed. curl error: {result.stderr}")

                self.log_message("Download successful using curl")
