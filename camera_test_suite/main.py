...
class CameraHardwareTester:
        ...
        for i in range(5):  # Check first 5 camera indices
            try:
                ...
                if cap.isOpened():
                    ...
                    cameras_found.append(f"Camera {i}: {width}x{height}")
                    self.log_message(f"Found camera {i}: {width}x{height}")
                cap.release()
            except Exception as e:
                self.log_message(f"Error checking camera {i}: {e}")

        if cameras_found:
            self.log_message(f"Found {len(cameras_found)} camera(s)")
    ...
    def connect_camera(self, index):
        ...
        try:
            ...
            if self.camera.isOpened():
                ...
                self.status_label.config(text=f"Status: Connected to camera {index}")
                self.log_message(f"Connected to camera {index}")
                self.start_preview()
            else:
                self.log_message(f"Failed to connect to camera {index}")
        except Exception as e:
            self.log_message(f"Error connecting to camera {index}: {e}")
    ...
    def manual_connect_camera(self):
        ...
        # Simple dialog for camera index
        index = tk.simpledialog.askinteger("Camera Index", "Enter camera index (0-9):",
    ...
    def disconnect_camera(self):
        ...
        if self.camera:
            ...
            self.camera = None
            self.camera_index = None
            self.status_label.config(text="Status: No camera connected")
